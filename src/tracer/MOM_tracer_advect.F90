...
subroutine advect_tracer(h_end, uhtr, vhtr, OBC, dt, G, GV, US, CS, Reg, &
      h_prev_opt, max_iter_in, x_first_in, uhr_out, vhr_out, h_out, flux_type)
...
subroutine advect_y(Tr, hprev, vhr, vh_neglect, OBC, domore_v, ntr, Idt, &
  ...
  if (usePLMslope) then
    ...
    enddo ; endif ; enddo ! End of m-, & j- loops.
  endif ! usePLMslope
  ...
  do j=js,je ; if (do_j_tr(j)) then
    ...
    do m=1,ntr
      if (advect_this_tracer(m)) then
        ...
        elseif (flux_type == 1) then
          if (associated(Tr(m)%ad_y_resolved)) then ; do i=is,ie ; if (do_i(i,j)) then
            Tr(m)%ad_y_resolved(i,J,k) = Tr(m)%ad_y_resolved(i,J,k) + flux_y(i,m,J)*Idt
          endif ; enddo ; endif
        elseif (flux_type == 2) then
          if (associated(Tr(m)%ad_y_param)) then ; do i=is,ie ; if (do_i(i,j)) then
